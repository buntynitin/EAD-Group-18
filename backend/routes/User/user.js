...
const { endpointValidation } = require('../../validation/endpoint_validation');
...
router.post('/addEndpoint',verify,async(req,res)=>{
    const { error } = endpointValidation(req.body)
    if (error)
        return res.status(400).json({ 'error': error.details[0].message })

    try {
        const endpointExists = await Endpoint.findOne({ user_id: mongoose.Types.ObjectId(req.user._id) })
        if (endpointExists){
            try{
               return await Endpoint.findOneAndUpdate({ user_id: mongoose.Types.ObjectId(req.user._id) },req.body)
            }catch{
               return res.status(400).json({ 'error': "Network error" })
            }
            ...
            const endpoint = new Endpoint({user_id: mongoose.Types.ObjectId(req.user._id),...req.body})
            try {
                await endpoint.save();
                return res.status(200).json({ 'message': "Endpoint Created" })
