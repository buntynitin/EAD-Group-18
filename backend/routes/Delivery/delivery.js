...
const { intransitnotification } = require('../../components/notification')
...
router.post('/sendDeliveryrequest', verify, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const agent_id = mongoose.Types.ObjectId(distanceMatrix[mindistanceindex].delivery_id)
            const request = new Request({
                deliveryagent_id: agent_id,
            ...
            try {
                await request.save();
                socket.emit('sendDeliveryRequest', {room_id: agent_id})

                return res.status(200).json({deliveryagent_id:mongoose.Types.ObjectId(distanceMatrix[mindistanceindex].delivery_id)})
            }
            catch {
...
router.post('/handTodelivery', verify, async (req, res) => {
    ...
    if (req.body.order_id && (req.body.order_id.length === 36)) {
        try {
            ...
            intransitnotification(order.user_id)
