...
function Request() {
    ...
    useEffect(() =>{
        socket.emit("join_room",{room_id:state._id})

        socket.on("getDeliveryRequest",(data) => {
            recallAPI()
        }) 
    ...
    const recallAPI = () =>{
        axios.get('http://192.168.29.82:3001/api/delivery/getRequest', {
            headers: {
    ...
    const handleScan = (data) => {
        ...
        if(data){
            ...
            if(data.length === 36)
            {
                axios.post('http://192.168.29.82:3001/api/delivery/handTodelivery',{order_id:data},{
                    headers:{
                        'auth-token': state.token
                    }
                }).then((res)=>{
                    setScanning(false)
                    socket.emit('sendUpdate2client', {room_id: data})
                    socket.emit('sendUpdate2restaurant', {room_id: data})
                                ...
                                <div>
                            ...
                            </Typography>
                                    {
                                        requestList.map((item, index) =>
                                            <Card key={index} style={{ borderRadius: "18px", marginBottom: "16px" }}>
                                                <Grid container>
                                                    <Grid item xs={2} sm={1}>
