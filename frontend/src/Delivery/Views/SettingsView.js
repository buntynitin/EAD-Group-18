...
function SettingsView() {
    ...
    useEffect(() => {
        axios.get('http://192.168.29.82:3001/api/delivery/getUserdetail', {
            headers: {
    ...
    const handleAdd = () => {
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                axios.get(`http://192.168.29.82:3001/api/delivery/geocode?lat=${position.coords.latitude}&long=${position.coords.longitude}`,
                )
                    .then(function (response) {
                        ...
                        axios.post('http://192.168.29.82:3001/api/delivery/updateLocation', addressobj, {
                            headers: {
                                'auth-token': state.token
                            }
                        }).then((res) => {
    ...
    return (
        <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", marginTop: "16px" }}>
                <Button style={{borderRadius:"14px"}} variant='contained' color="primary" onClick={handleLogout}>
