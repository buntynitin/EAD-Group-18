...
import {Typography,Grid,Card,Divider, Fab,Slide, Dialog,  TextField, Select, MenuItem,Button,DialogContent,DialogActions} from '@material-ui/core'
...
export default function OrderDetailView() {
    ...
    useEffect(() =>{
        ...
        socket.on("getMessageFromUser",(data) => { 
            recallAPI()
         }) 
         
         socket.on("receiveUpdateFromDelivery",(data) => { 
            recallAPI()
         })  
  ...
  const handleSave = () =>{
    ...
    {
        axios.post('http://192.168.29.82:3001/api/delivery/sendDeliveryrequest',{},{
            headers:{
                'auth-token' : state.token
            }
        }).then((res)=>{
            const myobj = {order_id:order.order_id,order_status:'Order Confirmed',change_payment : false}
            axios.post('http://192.168.29.82:3001/api/order/changestatus',myobj,{
                headers: {
                    'auth-token': state.token
                }
            }).then((response) => {
                // socket.emit('sendDeliveryRequest', {room_id: res.data.deliveryagent_id})
                recallAPI()

                // socket.emit('sendUpdate2client', {room_id: order.order_id})
                setSubmitting(false)
                setOpen1(false)
    ...
    axios.post('http://192.168.29.82:3001/api/order/changestatus',obj,{
  ...
  const handleSend = () =>{
    ...
    if(newMessage.replace(/\s/g,'') === '')
        ...
        axios.post('http://192.168.29.82:3001/api/order/addMessage',obj,{
                ...
                setOrder({...order,custom_chat:[...order.custom_chat,{by: 'restaurant', message: newMessage}]}) 
                scrollToBottom();
                setLoad(false)
                setNewMessage('')
